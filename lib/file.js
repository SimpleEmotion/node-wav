 ...
 * @param {boolean} [options.buffer=false] - true if byte buffer of sample data should be provided
...
function read( filename, options, done ) {
  ...
  fs.readFile( filename, function ( err, buffer ) {
    ...
    file.dataOffset = d;
    file.numSamples = file.dataChunkSize / file.blockAlign;
    file.duration = file.numSamples / file.sampleRate;

    if ( options.hash || options.buffer || options.data ) {
